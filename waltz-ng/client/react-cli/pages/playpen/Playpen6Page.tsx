...
import SubSection from "../../components/common/sub-section/SubSection";
...
import StaticPanel from "../../components/common/static-panel/StaticPanel";
import StaticPanels from "../../components/common/static-panel/StaticPanels";
...
const Playpen6Page = ({helloText}: DummyComponentProps) => {
  ...
  const toastsPlaceholders: ToastCreateType[] = [
    {
      ...
      message:
        "this is a toast lorem ipsum dolor sit amet alkjfgqsfua absfuasf  asfusagf abfuisfas bafiusgfas basfuigsafs asgfuisagf",
    },
    {
      type: NotificationTypeEnum.INFO,
      message:
        "this is a toast lorem ipsum dolor sit amet alkjfgqsfua absfuasf  asfusagf abfuisfas bafiusgfas basfuigsafs asgfuisagf alkjfgqsfua absfuasf  asfusagf abfuisfas bafiusgfas basfuigsafs asgfuisagf alkjfgqsfua absfuasf  asfusagf abfuisfas bafiusgfas basfuigsafs asgfuisagf",
    },
  ];

  const {isPending: newsPending, data: mdText} = useQuery(
    staticPanelApi.findByGroupKey("NEWS")
  );
  ...
  return (
    <div>
      <div style={{borderRadius: "4px", padding: "16px"}}>
        ...
        <p>{helloText}</p>
        <button className="btn btn-default" onClick={onIncrementRedux}>
        ...
        />
        <Toggle
          onToggle={() => setToggleState((prev) => !prev)}
          state={toggleState}
        ></Toggle>
        ...
        ></DateTime>
        <SubSection header={<p>Header</p>} content={<p>Content is here</p>} />
        <PersonList
          people={people}
          canAdd={true}
          canRemove={false}
        ...
        <PageHeader
          ...
          summary={<h4>This is a summary</h4>}
        ></PageHeader>
        <br />
        <Section icon="pencil-square-o" name="Static Panel" small="Static panel in here">
          {newsPending && <Loader />}
          {mdText && <StaticPanel panel={mdText[0]} showTitle={true} />}
        </Section>
        <Section icon={"bolt"} name={"Static Panels"} small={"Static panels in here"}>
          {showSecondToast && <Toast type={"ERROR"} message={"Hello, warned"} />}
          <StaticPanels groupKey={"HOME"} />
        </Section>
        <Toast
          type={toastsPlaceholders[0].type}
          message={toastsPlaceholders[0].message}
        />
      </div>
    </div>
  );
