...
const Home: React.FC = () => {
  //Complexity Kind
  const [selectedComplexityKinds, setSelectedComplexityKinds] = useState<any[]>([
    {
      name: "Mike",
  ...
  const onSelectComplexityKind = (complexityKind: TableRow) => {
    if (!selectedComplexityKinds.some((ck) => ck.id === complexityKind.id)) {
      setSelectedComplexityKinds([...selectedComplexityKinds, complexityKind]); // Append if not already selected
    }
  };
  ...
  const selectionFilterCost = (costKind: CostKind) => !costKindIds.includes(costKind.id);
  ...
  return (
    <div>
      <h1>Home Page</h1>
      <p>This is a simple React app using TypeScript and best practices.</p>
