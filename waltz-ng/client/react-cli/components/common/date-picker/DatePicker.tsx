...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  return (
    <div className={[styles.wrapper, className].filter(Boolean).join(" ")}>
      {mode === VisualStateModes.VIEW ? (
        <>
          <span className={styles.readonly}>{formattedValue}</span>
          {canEdit && (
            <Button className={styles.inlineBtn} onClick={onEdit}>
              <>
                <Icon name="edit" />
              </>
            </Button>
        ...
        <>
          <Flatpickr
            ...
            value={value}
            onChange={([d]: Date[]) => setValue(d)}
          />
          <Button className={styles.inlineBtn} onClick={onCancel}>
            <>
              <Icon name="times" />
            </>
          </Button>
