/*
 * Waltz - Enterprise Architecture
 * react-component: AngularJS directive to embed React components with dynamic prop binding
 * Inspired by svelte-component.js
 *
 * Supports offloading of routing to the React Application
 */

import React from "react";
import {createRoot} from "react-dom/client";
import {QueryClient, QueryClientProvider} from "@tanstack/react-query";
import _ from "lodash";
import {ToastProvider} from "../react-cli/context/toast/ToastProvider";
import ErrorBoundary from "../react-cli/ErrorBoundary";
import WaltzAppRouter from "../react-cli/router/WaltzAppRouter";

const queryClient = new QueryClient();

function calcParamKeys(attrs) {
  return _.chain(attrs)
    .map((p, k) => ({p, k}))
    .reject((d) => d.k.startsWith("$"))
    .value();
}

function calcInitialParams(paramKeys, scope) {
  return _.chain(paramKeys)
    .map((d) => Object.assign({}, d, {v: scope.$eval(d.p)}))
    .reduce((acc, d) => {
      acc[d.k] = d.v;
      return acc;
    }, {})
    .value();
}

const directive = function () {
  return {
    restrict: "E",
    link: (scope, elem, attrs) => {
      const paramKeys = calcParamKeys(attrs);
      let currentProps = calcInitialParams(paramKeys, scope);

      // Look up the React component from the scope using the 'component' attribute
      let ReactComponent = _.get(scope, attrs.component, undefined);
      let useReactRouter = attrs.router === "true";

      let root = null;

      function doRender() {
        ReactComponent = _.get(scope, attrs.component); // re-fetch in case it changes
        useReactRouter = attrs.router === "true";

        if (ReactComponent || useReactRouter) {
          if (!root) {
            root = createRoot(elem[0]);
          }
          root.render(
            <React.StrictMode>
              <ErrorBoundary>
                <QueryClientProvider client={queryClient}>
                  <ToastProvider>
                    {useReactRouter && <WaltzAppRouter />}
                    {!useReactRouter && <ReactComponent {...currentProps} />}
                  </ToastProvider>
                </QueryClientProvider>
              </ErrorBoundary>
            </React.StrictMode>
          );
        }
      }

      doRender();

      // Watch each param and update React props on change
      paramKeys.forEach(({p, k}) => {
        if (p.startsWith(".")) return;
        if (k === "router") return;
        scope.$watch(p, (newVal) => {
          if (_.isEqual(currentProps[k], newVal)) return;
          currentProps = Object.assign({}, currentProps, {[k]: newVal});
          doRender();
        });
      });

      // Watch for changes to the component reference itself
      if (attrs.component) {
        scope.$watch(attrs.component, () => {
          doRender();
        });
      }

      // Cleanup on destroy
      elem.on("$destroy", () => {
        if (root) root.unmount();
      });
    },
  };
};

export default directive;
