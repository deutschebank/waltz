...
@RestController
@RequestMapping("/data-extract/physical-flows/")
public class PhysicalFlowExtractorController  {
    ...
    @Autowired
    public PhysicalFlowExtractorController(DSLContext dsl) {
        this.dsl = dsl;
    }


    @PostMapping(value = "/all/{kind}/{id}")
    ...
    @PostMapping(value = "/produces/{kind}/{id}")
    public void exportProduces(HttpServletRequest request,
                             HttpServletResponse response,
                             @PathVariable("kind") String kind,
                             @PathVariable("id") long id) throws IOException {
        EntityReference ref = mkRef(EntityKind.valueOf(kind), id);
    ...
    @PostMapping(value = "/consumes/{kind}/{id}")
    public void exportConsumes(HttpServletRequest request,
                             HttpServletResponse response,
                             @PathVariable("kind") String kind,
                             @PathVariable("id") long id) throws IOException {
        EntityReference ref = mkRef(EntityKind.valueOf(kind), id);
        ...
        writeReportResults(
                response,
                preparePhysicalFlows(
                        prepareConsumesQuery(ref),
                        parseExtractFormat(request),
                        fileName,
                        getTagsMap()));
    }

    @PostMapping(value = "/by-selector")
    public void exportBySelector(HttpServletRequest request,
                                 HttpServletResponse response,
    ...
    private List<List<Object>> prepareReportRows(SelectConditionStep<Record> qry,
        ...
        return results
                ...
                .map(row -> {
                    ...
                    reportRow.add(physicalFlowTags == null ? "" : String.join(",", physicalFlowTags));
